...
class Models_AlertasCualitativa extends DB_Connect {
...
public function getListCualitativaAll()
{ 
...
where enviado=1 and cc.activo=1  AND l.idtipoLaft=1 
  ";
...
public function getListCountCualitativaAll()
{ 
  $iddocumento = $this->getValorIntSanit("iddocumento");
  $idtipoLaft = $this->getValorIntSanit("idtipoLaft");
  $idColegio = $this->getValorIntSanit("idColegio");
...
public function getComentariosList($data)
{
...
to_char(fecharegistro,'dd/mm/YYYY') as fecharegistro,
to_char(fecharegistro,'hh24:mi:ss') as horaregistro
FROM ocpreporte.comentario_alerta c inner join 
ocpreporte.estado_alerta e on c.idestadoocp=e.id
where idalerta=".$idalerta." and IDTIPOALERTA=".$idtipoalerta." order by id desc";
...
public function addCalificacion($objRequest)
{
      

    $comentario=isset($objRequest->comentario)?$objRequest->comentario:"";
    $descripcion=isset($objRequest->descripcion)?$objRequest->descripcion:"";
    $idestado=isset($objRequest->idestado)?$objRequest->idestado:"";
    $idalerta=isset($objRequest->idalerta)?$objRequest->idalerta:"";
    $idtipoalerta=isset($objRequest->idtipoalerta)?$objRequest->idtipoalerta:"";
    
   
        $db=$this->connect(); 
        $sqlInsert="INSERT INTO ocpreporte.comentario_alerta
        (IDALERTA,COMENTARIO,FECHAREGISTRO,IDESTADOOCP,IDTIPOALERTA
        ) VALUES 
        ...
        oci_bind_by_name($stInsert, ":idalerta", $idalerta);
        oci_bind_by_name($stInsert, ":comentario", $comentario);
        oci_bind_by_name($stInsert, ":idestado", $idestado);
        oci_bind_by_name($stInsert, ":idtipoalerta", $idtipoalerta);

        oci_execute($stInsert);  
        oci_free_statement($stInsert);
        oci_close($db);

        $response=array("response"=>"correcto","estado"=>true);      
        return ($response);  
