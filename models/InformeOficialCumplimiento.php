...
class Models_InformeOficialCumplimiento extends DB_Connect {
...
public function getListInformeAll()
{ 
...
//die("=>".$pageSize);
  $sql="
   select d.id,lpad(d.codigo,5,'0') as codigo,n.descripcion as notaria,
        c.nombre as colegio,idestadoocp,
            NVL((
        select ea.descripcion from ocpreporte.comentario_alerta ca 
        inner join ocpreporte.estado_alerta ea on ca.idestadoocp=ea.id 
         where ca.idalerta=d.id and ca.idtipoalerta=4 ORDER BY ca.id desc FETCH NEXT 1 ROWS ONLY  
    ),'PENDIENTE') as estado_ocp,
        
        to_char(d.fecharegistro,'dd/mm/YYYY') as fecharegistro,
        TO_CHAR(d.fecharegistro, 'HH24:MI:SS') as horaregistro
    ...
    if($idEstadoOcp>0)
            {
              ...
              if($idEstadoOcp==1)
              {
                ...
                where ca.idalerta=d.id and ca.idtipoalerta=4 
                )=0  OR ";
              }
              $sql.="
                ( select ca.idestadoocp from ocpreporte.comentario_alerta ca 
                where ca.idalerta=d.id and ca.idtipoalerta=4   ORDER BY ca.id desc 
...
where enviado=1 and cc.activo=1 and cc.vinculado=1 AND l.idtipoLaft=2 
  ";
    ...
    if($idEstadoOcp>0)
            {
              ...
              if($idEstadoOcp==1)
              {
                  ...
                  $sql.=" 
                ( select count(1) from ocpreporte.comentario_alerta ca 
                where ca.idalerta=cc.id and ca.idtipoalerta=2 
                )=0  OR ";
              }
              $sql.="
                ( select ca.idestadoocp from ocpreporte.comentario_alerta ca 
