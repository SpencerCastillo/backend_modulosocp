...
class Models_NotariaInstrumento extends DB_Connect {
    ...
    public function verHojaCotejo($data)
    {
      $data=json_decode($data);
      $idDocumento=isset($data->id)?$data->id:"0";
    ...
    $db=$this->connect();
    $texto=$this->getHtmlPdfInformacion($db,$idDocumento,$data);
     ...
     // $this->guardarDocumentoActa($contenido,$db);
      ...
      header("Content-Disposition: inline; filename=hoja_cotejo.pdf");
 ...
 private function getHtmlPdfInformacion($db,$id,$data)
  {
    ...
    include 'HtmlFormatoHojaCotejo.php';
    $d=new HtmlFormatoHojaCotejo;
     ...
     */
    public function getInstrumentoByNotariaAndPeticion($get_data)
        {        
           $info=json_decode($get_data);
           $rucNotaria=$info->ruc_notaria;
           $idPeticion=$info->idpeticion;  
           $oficio=$info->oficio;  
                   
           $db=$this->connect();
           $sql="SELECT H.NUMERODEKARDEX,
            LISTAGG(  NOMBRECONTRATO, ', ') WITHIN GROUP (ORDER BY P.ID) AS NOMBRECONTRATO
           ,NUMERO,FECHAAUTORIZACION AS FECHAINSTRUMENTO,IDINSTRUMENTO,IDINSTRUMENTO AS IDTIPOINSTRUMENTO,
              LISTAGG(P.ID, ', ') WITHIN GROUP (ORDER BY P.ID) AS IDPETICION
            FROM DATAHISTORICA.PETICION P INNER JOIN DATAHISTORICA.HISTORY_INSTRUMENTO_RPT H
            ON P.ID=H.IDPETICION
            WHERE RUC_NOTARIA='".$rucNotaria."'
            AND CARPETAFISCAL='".$oficio."' AND P.ESTADO=1
            GROUP BY (H.NUMERODEKARDEX,NUMERO,FECHAAUTORIZACION,IDINSTRUMENTO)
            ";
            ...
            while (($row = oci_fetch_assoc($stid)) != false) {
                $row["IDINSTRUMENTO"]=DB_Connect::getTipoInstrumentoParaReporte($row["IDINSTRUMENTO"]);
                $row["NUMROW"]=$k+1;
                $all[]=$row;
            $k++;
            }
